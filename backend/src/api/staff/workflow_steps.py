...
async def get_step_detail(
    ...
    if step.dossier_id:
        dos_result = await db.execute(
            ...
            .where(Dossier.id == step.dossier_id)
        )
        dossier = dos_result.scalar_one_or_none()
        if not dossier:
            raise HTTPException(404, "Dossier not found")

        # Build pages from all dossier documents
        page_list = []
        for doc in dossier.documents:
            for p in sorted(doc.scanned_pages, key=lambda x: x.page_number):
                page_list.append({
                    "page_number": p.page_number,
                    "image_url": _resolve_image_url(p.image_oss_key),
                    "ocr_text": p.ocr_corrected_text or p.ocr_raw_text,
                    "ocr_confidence": p.ocr_confidence,
                    "document_name": doc.document_type.name if doc.document_type else None,
                })
        ...
        all_steps_result = await db.execute(
            select(WorkflowStep)
            .options(selectinload(WorkflowStep.annotations), selectinload(WorkflowStep.department))
            .where(WorkflowStep.dossier_id == dossier.id)
